...
export default async function Projects() {
  ...
  const { data: dbProjects } = supabase ? await supabase.from("projects").select("*, tasks(id, status)").order("created_at", { ascending: false }) : { data: [] };
  const { data: dbClients } = supabase ? await supabase.from("clients").select("id, display_name") : { data: [] };

  const projects = (dbProjects || []).map((p) => {
    const total = p.tasks?.length || 0;
    const completed = p.tasks?.filter((task: { status: string }) => task.status === "completed").length || 0;
    return {
      name: p.name,
      status: p.status,
